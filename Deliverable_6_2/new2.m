    ...
    methods
        function obj = NmpcControl(car, H)
            ...
            for k =1:N
                ...
                cost = cost + 0.5*(obj.X(2,k)-obj.ref(1))*(obj.X(2,k)-obj.ref(1))';
            ...
            for k = 1:N
                ...
                opti.subject_to(-0.087<=obj.X(3,k)<=0.087);
            ...
            h = diag([1/(car.length+3)^2, 1/(car.width+1)^2]);  % Collision avoidance matrix
            x_other = obj.x0other(1:2);
            v_other = obj.x0other(4);
            for k = 1:N
                opti.subject_to((obj.X(1:2,k) - x_other)' * h * (obj.X(1:2,k) - x_other) >= 1);  % Collision avoidance constraint
                x_other(1) = x_other(1) + v_other * car.Ts;
            end
